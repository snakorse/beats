...
func New(
	...
	matcher, err := matcherType(config.Patterns)
	if err != nil {
		return nil, err
	}
...
func (mlr *Reader) Close() error {
	return mlr.reader.Close()
}
...
func getStream(str common.MapStr) string {
	data, err := str.GetValue("stream")
	if err != nil {
		return ""
	}
	val, ok := data.(string)
...
func (mlr *Reader) readFirst() (reader.Message, error) {
	for {
		...
		if mlr.pred(mlr.last, message.Content) {
			mlr.isMerging = true
		}
		mlr.load(message)
...
func (mlr *Reader) readNext() (reader.Message, error) {
	for {
		...
		isMatch := mlr.pred(mlr.last, message.Content)
		if getStream(mlr.message.Fields) != getStream(message.Fields) {
			msg := mlr.finalize()
			mlr.load(message)
			mlr.isMerging = isMatch
			return msg, nil
		}
...
func afterMatcher(pats []match.Matcher) (matcher, error) {
	return genPatternMatcher(pats, func(last, current []byte) []byte {
		return current
	})
}

func beforeMatcher(pats []match.Matcher) (matcher, error) {
	return genPatternMatcher(pats, func(last, current []byte) []byte {
		return last
	})
}
